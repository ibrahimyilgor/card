  ...
  useEffect(() => {
    if (flashcards.length > 0) {
      ...
      const pairs = flashcards.slice(0, 6).flatMap((card, index) => [
        { id: `front-${index}`, pairId: index, text: card.front_text, type: 'front' },
        { id: `back-${index}`, pairId: index, text: card.back_text, type: 'back' },
      ]);
      ...
      if (firstCard.pairId === secondCard.pairId && firstCard.type !== secondCard.type) {
        ...
        setTimeout(() => {
          ...
          if (matched.length + 2 === cards.length) {
            setTimeout(() => {
              onComplete?.({ attempts: attempts + 1, totalPairs: flashcards.length });
            }, 500);
          }
        }, 500);
      } else {
    ...
    <Box sx={{ width: '100%', maxWidth: 800, mx: 'auto' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', gap: 4, mb: 4 }}>
        <Box sx={{ textAlign: 'center' }}>
          <Typography variant="caption" sx={{ color: 'text.cardSubtitle', fontFamily: 'Inter, sans-serif' }}>
            {t('matches') || 'Matches'}
          </Typography>
          <Typography variant="h5" sx={{ color: 'text.cardTitle', fontWeight: 700, fontFamily: 'Inter, sans-serif' }}>
      ...
      >
        {cards.map((card) => (
          <MotionBox
            ...
            animate={{ 
              scale: 1,
              rotateY: isFlipped(card.id) ? 180 : 0,
            ...
            >
              ...
              <Box
                sx={{
                  ...
                  '&:hover': !isMatched(card.id) && !isChecking ? {
              ...
              >
                ...
                >
                  {card.type === 'front' ? (t('question') || 'Q') : (t('answer') || 'A')}
                </Typography>
              </Box>
            </Box>
          </MotionBox>
