...
export default function GameSummary({ 
  ...
  const getGrade = () => {
    ...
    if (gameMode === 'survival' && livesRemaining === 0) {
      return { grade: '💀', message: t('game_over') || 'Game Over!', color: '#ef4444' };
    }
    
    if (percentage >= 90) return { grade: 'A+', message: t('outstanding') || 'Outstanding!', color: '#22c55e' };
    if (percentage >= 80) return { grade: 'A', message: t('excellent') || 'Excellent!', color: '#22c55e' };
    if (percentage >= 70) return { grade: 'B', message: t('good_job') || 'Good job!', color: '#3b82f6' };
    if (percentage >= 60) return { grade: 'C', message: t('keep_practicing') || 'Keep practicing!', color: '#f59e0b' };
    return { grade: 'D', message: t('need_more_practice') || 'Need more practice', color: '#ef4444' };
  };
      ...
      >
        <Box
          sx={{
            ...
            alignItems: 'center',
            justifyContent: 'center',
            background: `linear-gradient(135deg, ${alpha(gradeColor, 0.2)} 0%, ${alpha(gradeColor, 0.05)} 100%)`,
