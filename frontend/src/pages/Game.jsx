...
import { getGameFlashcards, getHardFlashcards, getFlashcardsWithOptions, validateAnswer, updateFlashcardStats } from '../services/gameServices';
...
import { TimerDisplay, LivesDisplay, WriteInput, MultipleChoice, MatchGrid } from '../components/game';
...
export default function Game({ onBackToDecks }) {
  ...
  const settings = location.state?.settings || { mode: 'standard', timeLimit: 10, lives: 3 };
  ...
  const [gameStats, setGameStats] = useState({ timeSpent: 0, matchAttempts: 0 });
  ...
  const handleAnswer = useCallback(
    async (isCorrect) => {
      ...
      if (flashcards[currentCardIndex]?.id) {
        try {
          await updateFlashcardStats(flashcards[currentCardIndex].id, isCorrect);
        } catch (err) {
  ...
  }, [gameEnded, loading, flashcards, gameMode, writeResult, showChoiceResult, currentCardIndex, handleCardFlip, handleAnswer]);
  ...
  if (flashcards.length === 0) {
    return (
      <PageContainer centered>
        <EmptyState
          icon={StyleIcon}
          title={gameMode === 'hard_cards' 
            ? (t('no_hard_cards') || 'No hard cards found')
            : (t('no_flashcards') || 'No flashcards found')}
          description={gameMode === 'hard_cards'
            ? (t('no_hard_cards_desc') || 'Great job! You have no cards marked as difficult.')
            : (t('no_flashcards_desc') || 'This deck has no flashcards yet. Add some cards to start studying.')}
      ...
      >
        ...
        >
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            {gameMode === 'timed' && (
              <TimerDisplay timeLeft={timer.timeLeft} totalTime={settings.timeLimit} />
            )}
            {gameMode === 'survival' && (
