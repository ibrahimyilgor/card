...
module.exports = (pool) => {
    ...
    router.get('/overview', authenticateToken, async (req, res) => {
        ...
        try {
            ...
            res.json({
                ...
                totalCardsStudied: parseInt(sessionStats.rows[0].total_cards_studied) || 0,
                ...
                avgSessionDuration: Math.round(parseFloat(avgSession.rows[0].avg_duration) || 0)
            });
        } catch (err) {
    ...
    router.get('/deck/:deckId', authenticateToken, async (req, res) => {
        ...
        try {
            // Verify ownership
            const ownership = await pool.query('SELECT account_id FROM deck WHERE id = $1', [deckId]);
            if (ownership.rows.length === 0) return res.status(404).json({ error: 'Deck not found' });
            if (ownership.rows[0].account_id !== accountId) return res.status(403).json({ error: 'Access denied' });
    ...
    router.get('/cards/:deckId', authenticateToken, async (req, res) => {
        ...
        try {
            ...
            const validSorts = ['id', 'correct_count', 'wrong_count', 'accuracy', 'created_at'];
    ...
    router.post('/session', authenticateToken, async (req, res) => {
        const accountId = req.user.accountId;
        const { deckId, gameMode, cardsStudied, correctAnswers, wrongAnswers, durationSeconds } = req.body;
        ...
        try {
            ...
            const result = await pool.query(`
                ...
                RETURNING *
            `, [accountId, deckId, gameMode, cardsStudied, correctAnswers, wrongAnswers, durationSeconds]);
    ...
    router.get('/insights', authenticateToken, async (req, res) => {
        ...
        try {
            ...
            res.json({
                ...
                weeklyComparison: weeklyComparison.rows.reduce((acc, row) => {
                    acc[row.week] = {
                    ...
                    return acc;
                }, { current: { cards: 0, correct: 0, wrong: 0 }, previous: { cards: 0, correct: 0, wrong: 0 } }),
