...
module.exports = (pool) => {
        ...
        try {
            ...
            const flashcardsWithOptions = flashcards.map((card, index) => {
                ...
                const options = [
                    ...
                    ...shuffledOthers.map(c => ({ text: c.back_text, isCorrect: false }))
                ];
            ...
            res.status(500).json({ error: 'Failed to fetch flashcards with options' });
        }
    });
    ...
    router.post('/validate-answer', authenticateToken, async (req, res) => {
        ...
        try {
            const result = await pool.query('SELECT back_text FROM flashcard WHERE id = $1', [flashcardId]);
